  ...
  [server(:gw1), server(:gw2)].each do |s|
    describe s do
      ...
      it "shows the IPSec flows" do
        ...
        expect(result).to match(%r{^flow esp in from #{dst} to #{src} peer #{dst} srcid #{src}/32 dstid #{dst}/32 type use})
        expect(result).to match(%r{^flow esp out from #{src} to #{dst} peer #{dst} srcid #{src}/32 dstid #{dst}/32 type require})
      end
      ...
      it "shows the IPSec SA" do
        ...
        expect(result.split("\n").length).to satisfy("be either 2 or 4") { |v| (v == 2) || (v == 4) }
      end
    end
  end
end
